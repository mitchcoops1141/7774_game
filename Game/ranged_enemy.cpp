	...
	: Game_Object(id, _walkingTextureID)
{
	...
	_frame_duration_milliseconds = 200;
...
void Ranged_Enemy::simulate_AI(Uint32 milliseconds_to_simulate, Assets* assets, Input* input, Scene* scene, SDL_Renderer*)
{
	...
	case State::shooting: //while state is agro
		...
		{
			//create spit ball!
			scene->add_game_object(new Spit_Ball(_ballID, scene, Vector_2D(_translation.x() +_width / 2, _translation.y() + _height / 2))); //add projectile object
			_ballCounter++; //add one to the ball counter
			_ballID = "SpitBall"; //reset the ball id
...
void Ranged_Enemy::render(Uint32 milliseconds_to_simulate, Assets* assets, SDL_Renderer* renderer, Configuration* config, Scene* scene)
{
...
void Ranged_Enemy::set_hp(int hp)
{
	_hp = hp; //set hp
...
void Ranged_Enemy::push_state(State state, Assets* assets, Input* input)
{
...
void Ranged_Enemy::pop_state(Assets* assets, Input* input)
{
	handle_exit_state(_state.top(), assets);
...
void Ranged_Enemy::handle_enter_state(State state, Assets* assets, Input*)
{
	Animated_Texture* texture = (Animated_Texture*)assets->get_asset(_texture_id);
	switch (state)
	{
...
void Ranged_Enemy::handle_exit_state(State state, Assets* assets)
{
	Animated_Texture* texture = (Animated_Texture*)assets->get_asset(_texture_id);
	switch (state)
	{
